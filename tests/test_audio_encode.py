...
async def test_produces_a_real_mp3():
    mp3 = await wav_to_mp3(tone_wav(1.0))
...
async def test_padding_silence_is_trimmed():
    ...
    duration = await mp3_duration_ms(await wav_to_mp3(padded))
...
async def test_duration_describes_the_encoded_clip_not_the_source():
    ...
    duration = await mp3_duration_ms(await wav_to_mp3(long_tail))
    assert duration is not None
...
async def test_duration_of_garbage_is_none_not_an_error():
    ...
    assert await mp3_duration_ms(b"not an mp3 at all") is None
